import { DID } from '../../did/register/did-register'
import { ERR_REQUIRE, ERROR, INVALID, KEY_TYPE, VC_STATUS } from '../../consts'
import { DIDNonce } from '../../did/register/nonce'
import { VC } from '../register/vc'
import { Gen } from '../../verification/vc-vp-verify/gen-vc-vp'
import { VCStatus } from './status'
import { v4 as uuidv4 } from 'uuid'

const faker = require('faker')

let getInitState: any = () => ({
  keyId: '',
  cid: '',
  cid2: '',
  didKey1: [],
  didKey2: [],
  didKey3: [],
  didKey4: [],
  issuanceDate: '',
  data: {
    did: '',
    nonce: '',
    did2: '',
    nonce2: ''
  },
  issuer: {
    did: ''
  },
  didId: '',
  vc: {
    version: '',
    version2: ''
  },
  issuerKey1: '',
  issuerKey2: ''
})

describe('Add VC Tags', () => {

  const state = getInitState()
  jest.setTimeout(20000)

  beforeEach(() => {
    state.didKey1 = []
    state.issuerKey1 = []
  })

  test('Add tags to only one cid with only one tag', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(200)
      expect(addTags.data.vcs[0].cid).toBe(state.cid)
      expect(addTags.data.vcs[0].status).toBe(VC_STATUS.ACTIVE)
      expect(addTags.data.vcs[0].did_address).toBe(state.issuer.did)
      expect(addTags.data.vcs[0].vc_hash).toBe(jwtVc)
      expect(addTags.data.vcs[0].tags).toMatchObject(tags)

      const vcGetStatus2 = await VCStatus.Get(state.cid)
      console.log('Get VC Status2: ', JSON.stringify(vcGetStatus2.data, null, 2))
      expect(vcGetStatus2.status).toEqual(200)
      expect(vcGetStatus2.data.cid).toBe(state.cid)
      expect(vcGetStatus2.data.status).toBe(VC_STATUS.ACTIVE)
      expect(vcGetStatus2.data.did_address).toBe(state.issuer.did)
      expect(vcGetStatus2.data.vc_hash).toBe(jwtVc)
      expect(vcGetStatus2.data.tags).toMatchObject(tags)
      expect(vcGetStatus2.data.revoked_at).toBe(null)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).not.toBeTruthy()
    }
  })

  test('Add tags to only one cid with multiple tags', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC1`
      const tag2 = tag1 + '_2'
      const tags = [tag1, tag2]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(200)
      expect(addTags.data.vcs[0].cid).toBe(state.cid)
      expect(addTags.data.vcs[0].status).toBe(VC_STATUS.ACTIVE)
      expect(addTags.data.vcs[0].did_address).toBe(state.issuer.did)
      expect(addTags.data.vcs[0].vc_hash).toBe(jwtVc)
      expect(addTags.data.vcs[0].tags).toMatchObject(tags)

      const vcGetStatus2 = await VCStatus.Get(state.cid)
      console.log('Get VC Status2: ', JSON.stringify(vcGetStatus2.data, null, 2))
      expect(vcGetStatus2.status).toEqual(200)
      expect(vcGetStatus2.data.cid).toBe(state.cid)
      expect(vcGetStatus2.data.status).toBe(VC_STATUS.ACTIVE)
      expect(vcGetStatus2.data.did_address).toBe(state.issuer.did)
      expect(vcGetStatus2.data.vc_hash).toBe(jwtVc)
      // expect(vcGetStatus2.data.tags).toMatchObject(tags)
      expect(vcGetStatus2.data.revoked_at).toBe(null)
    } catch (err) {
      console.log(err.response)
      expect(err).not.toBeTruthy()
    }
  })

  test('Add tags to multiple cids with only one tag', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const vcRegister2 = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register2: ', JSON.stringify(vcRegister2.data, null, 2))
      expect(vcRegister2.status).toEqual(201)
      state.cid2 = vcRegister2.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce4 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 4: ', JSON.stringify(nonce4.data, null, 2))
      expect(nonce4.status).toEqual(200)
      state.data.nonce = nonce4.data.nonce

      const schemaType2 = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject2 = { 'example_string': 'Tony' }
      const jwtVc2 = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType2, schemaType2, state.issuanceDate, vcSubject2)

      const vcAddStatus2 = await VCStatus.Add(state.cid2, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc2)
      console.log('Add VC Status2: ', JSON.stringify(vcAddStatus2.data, null, 2))

      const nonce5 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 5: ', JSON.stringify(nonce5.data, null, 2))
      expect(nonce5.status).toEqual(200)
      state.data.nonce = nonce5.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC1`
      const tags = [tag1]
      const cids = [state.cid, state.cid2]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(200)
      expect(addTags.data.vcs[0].cid).toBe(state.cid)
      expect(addTags.data.vcs[0].status).toBe(VC_STATUS.ACTIVE)
      expect(addTags.data.vcs[0].did_address).toBe(state.issuer.did)
      expect(addTags.data.vcs[0].vc_hash).toBe(jwtVc)
      expect(addTags.data.vcs[0].tags).toMatchObject(tags)
      expect(addTags.data.vcs[1].cid).toBe(state.cid2)
      expect(addTags.data.vcs[1].status).toBe(VC_STATUS.ACTIVE)
      expect(addTags.data.vcs[1].did_address).toBe(state.issuer.did)
      expect(addTags.data.vcs[1].vc_hash).toBe(jwtVc2)
      expect(addTags.data.vcs[1].tags).toMatchObject(tags)

      const vcGetStatus2 = await VCStatus.Get(state.cid)
      console.log('Get VC Status2: ', JSON.stringify(vcGetStatus2.data, null, 2))
      expect(vcGetStatus2.status).toEqual(200)
      expect(vcGetStatus2.data.cid).toBe(state.cid)
      expect(vcGetStatus2.data.status).toBe(VC_STATUS.ACTIVE)
      expect(vcGetStatus2.data.did_address).toBe(state.issuer.did)
      expect(vcGetStatus2.data.vc_hash).toBe(jwtVc)
      expect(vcGetStatus2.data.tags).toMatchObject(tags)
      expect(vcGetStatus2.data.revoked_at).toBe(null)
    } catch (err) {
      console.log(err.response)
      expect(err).not.toBeTruthy()
    }
  })

  test('Add tags to multiple cids with multiple tags', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const vcRegister2 = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register2: ', JSON.stringify(vcRegister2.data, null, 2))
      expect(vcRegister2.status).toEqual(201)
      state.cid2 = vcRegister2.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce4 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 4: ', JSON.stringify(nonce4.data, null, 2))
      expect(nonce4.status).toEqual(200)
      state.data.nonce = nonce4.data.nonce

      const schemaType2 = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject2 = { 'example_string': 'Tony' }
      const jwtVc2 = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType2, schemaType2, state.issuanceDate, vcSubject2)

      const vcAddStatus2 = await VCStatus.Add(state.cid2, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc2)
      console.log('Add VC Status2: ', JSON.stringify(vcAddStatus2.data, null, 2))

      const nonce5 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 5: ', JSON.stringify(nonce5.data, null, 2))
      expect(nonce5.status).toEqual(200)
      state.data.nonce = nonce5.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC1`
      const tag2 = tag1 + '_2'
      const tags = [tag1, tag2]
      const cids = [state.cid, state.cid2]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(200)
      expect(addTags.data.vcs[0].cid).toBe(state.cid)
      expect(addTags.data.vcs[0].status).toBe(VC_STATUS.ACTIVE)
      expect(addTags.data.vcs[0].did_address).toBe(state.issuer.did)
      expect(addTags.data.vcs[0].vc_hash).toBe(jwtVc)
      expect(addTags.data.vcs[0].tags).toMatchObject(tags)
      expect(addTags.data.vcs[1].cid).toBe(state.cid2)
      expect(addTags.data.vcs[1].status).toBe(VC_STATUS.ACTIVE)
      expect(addTags.data.vcs[1].did_address).toBe(state.issuer.did)
      expect(addTags.data.vcs[1].vc_hash).toBe(jwtVc2)
      expect(addTags.data.vcs[1].tags).toMatchObject(tags)

      const vcGetStatus2 = await VCStatus.Get(state.cid)
      console.log('Get VC Status2: ', JSON.stringify(vcGetStatus2.data, null, 2))
      expect(vcGetStatus2.status).toEqual(200)
      expect(vcGetStatus2.data.cid).toBe(state.cid)
      expect(vcGetStatus2.data.status).toBe(VC_STATUS.ACTIVE)
      expect(vcGetStatus2.data.did_address).toBe(state.issuer.did)
      expect(vcGetStatus2.data.vc_hash).toBe(jwtVc)
      expect(vcGetStatus2.data.tags).toMatchObject(tags)
      expect(vcGetStatus2.data.revoked_at).toBe(null)
    } catch (err) {
      console.log(err.response)
      expect(err).not.toBeTruthy()
    }
  })

  test('Add tags - Send request by non issuer', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const didRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('DID Register: ', JSON.stringify(didRegister.request.data, null, 2))
      expect(didRegister.request.status).toEqual(201)
      state.data.did = didRegister.request.data.id
      state.didKey1 = didRegister.didKey1

      const nonce = await DIDNonce.getDIDNonce(state.data.did)
      console.log('Nonce: ', JSON.stringify(nonce.data, null, 2))
      expect(nonce.status).toEqual(200)
      state.data.nonce2 = nonce.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.data.did,
        state.didKey1, state.data.nonce2)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(200)
      expect(addTags.data.vcs[0].cid).toBe(state.cid)
      expect(addTags.data.vcs[0].status).toBe(VC_STATUS.ACTIVE)
      expect(addTags.data.vcs[0].did_address).toBe(state.issuer.did)
      expect(addTags.data.vcs[0].vc_hash).toBe(jwtVc)
      expect(addTags.data.vcs[0].tags).toMatchObject(tags)
    } catch (err) {
      console.log(err.response)
      expect(err).not.toBeTruthy()
    }
  })

  test('Add tags - Send request again after added', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(200)
      expect(addTags.data.vcs[0].cid).toBe(state.cid)
      expect(addTags.data.vcs[0].status).toBe(VC_STATUS.ACTIVE)
      expect(addTags.data.vcs[0].did_address).toBe(state.issuer.did)
      expect(addTags.data.vcs[0].vc_hash).toBe(jwtVc)
      expect(addTags.data.vcs[0].tags).toMatchObject(tags)

      const nonce4 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 4: ', JSON.stringify(nonce4.data, null, 2))
      expect(nonce4.status).toEqual(200)
      state.data.nonce = nonce4.data.nonce

      const addTags2 = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags2.data, null, 2))
      expect(addTags2.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.VC_ALREADY_TAGGED.CODE)
      expect(err.response.data.message).toBe(INVALID.VC_ALREADY_TAGGED.MESSAGE)
    }
  })

  test('Add tags - Add tags before add status to active', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.VC_NOT_ACTIVE_TO_TAG.CODE)
      expect(err.response.data.message).toBe(INVALID.VC_NOT_ACTIVE_TO_TAG.MESSAGE)
    }
  })

  test('Add tags - Add tags after update status to revoke', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))
      expect(vcAddStatus.status).toEqual(200)

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const vcUpdateStatus1 = await VCStatus.Update(state.cid, state.cid, state.issuer.did, VC_STATUS.REVOKE, state.issuerKey1, state.data.nonce)
      console.log('Update VC Status1: ', JSON.stringify(vcUpdateStatus1.data, null, 2))
      expect(vcUpdateStatus1.status).toEqual(200)

      const nonce4 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 4: ', JSON.stringify(nonce4.data, null, 2))
      expect(nonce4.status).toEqual(200)
      state.data.nonce = nonce4.data.nonce

      const tags = ['Personal_VC']
      const addTags = await VCStatus.Tags([state.cid], tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.VC_NOT_ACTIVE_TO_TAG.CODE)
      expect(err.response.data.message).toBe(INVALID.VC_NOT_ACTIVE_TO_TAG.MESSAGE)
    }
  })

  test('Add tags - Send request with incorrect did_address', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const dummyDid = `did:idin:${uuidv4()}`
      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, dummyDid,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields.did_address.code).toBe(ERROR.DID_ADDRESS.CODE)
      expect(err.response.data.fields.did_address.message).toBe(ERROR.DID_ADDRESS.MESSAGE)
    }
  })

  test('Add tags - Send request with incorrect nonce', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields.nonce.code).toBe(ERROR.NONCE.CODE)
      expect(err.response.data.fields.nonce.message).toBe(ERROR.NONCE.MESSAGE)
    }
  })

  test('Add tags - Send request with incorrect cids', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [uuidv4()]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields['cid[0]'].code).toBe(INVALID.CID_IS_INVALID.CODE)
      expect(err.response.data.fields['cid[0]'].message).toBe(INVALID.CID_IS_INVALID.MESSAGE)
    }
  })

  test('Add tags - Send request with correct & incorrect cids', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [uuidv4(), state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields['cid[0]'].code).toBe(INVALID.CID_IS_INVALID.CODE)
      expect(err.response.data.fields['cid[0]'].message).toBe(INVALID.CID_IS_INVALID.MESSAGE)
    }
  })

  test('Add tags - Send request without cids', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = ''
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields.cids.code).toBe(INVALID.CIDS_TO_ADD_TAG.CODE)
      expect(err.response.data.fields.cids.message).toBe(INVALID.CIDS_TO_ADD_TAG.MESSAGE)
    }
  })

  test('Add tags - Send request without tags', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const tags = ''
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields.tags.code).toBe(INVALID.TAGS_TO_ADD_TAG.CODE)
      expect(err.response.data.fields.tags.message).toBe(INVALID.TAGS_TO_ADD_TAG.MESSAGE)
    }
  })

  test('Add tags - Send request without did_address', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, '',
        state.issuerKey1, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields.did_address.code).toBe(ERR_REQUIRE.DID_ADDRESS.CODE)
      expect(err.response.data.fields.did_address.message).toBe(ERR_REQUIRE.DID_ADDRESS.MESSAGE)
    }
  })

  test('Add tags - Send request without nonce', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey1, '')
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.PARAMS.CODE)
      expect(err.response.data.message).toBe(INVALID.PARAMS.MESSAGE)
      expect(err.response.data.fields.nonce.code).toBe(ERR_REQUIRE.NONCE.CODE)
      expect(err.response.data.fields.nonce.message).toBe(ERR_REQUIRE.NONCE.MESSAGE)
    }
  })

  test('Add tags - Send request with incorrect signature', async () => {
    try {
      const issRegister = await DID.Register(KEY_TYPE.EcdsaSecp256r1VerificationKey2019)
      console.log('Issuer Register: ', JSON.stringify(issRegister.request.data, null, 2))
      expect(issRegister.request.status).toEqual(201)
      state.issuer.did = issRegister.request.data.id
      state.issuerKey1 = issRegister.didKey1
      state.issuerKey2 = issRegister.didKey2
      state.keyId = issRegister.request.data.verificationMethod[0].id

      const nonce1 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 1: ', JSON.stringify(nonce1.data, null, 2))
      expect(nonce1.status).toEqual(200)
      state.data.nonce = nonce1.data.nonce

      const vcRegister = await VC.Register(state.issuer.did, state.issuerKey1, state.data.nonce)
      console.log('VC Register: ', JSON.stringify(vcRegister.data, null, 2))
      expect(vcRegister.status).toEqual(201)
      state.cid = vcRegister.data.cid

      const vcGetStatus1 = await VCStatus.Get(state.cid)
      console.log('Get VC Status1: ', JSON.stringify(vcGetStatus1.data, null, 2))
      expect(vcGetStatus1.status).toEqual(200)
      expect(vcGetStatus1.data.status).toBe(null)
      state.issuanceDate = vcGetStatus1.data.created_at

      const nonce2 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 2: ', JSON.stringify(nonce2.data, null, 2))
      expect(nonce2.status).toEqual(200)
      state.data.nonce = nonce2.data.nonce

      const schemaType = faker.name.firstName() + `'sDocument` + '_Type'
      const vcSubject = { 'example_string': 'Tony' }
      const jwtVc = await Gen.VC(state.cid, state.issuer.did, state.issuer.did, state.issuerKey1,
        state.keyId, state.schemaId, schemaType, schemaType, state.issuanceDate, vcSubject)

      const vcAddStatus = await VCStatus.Add(state.cid, state.issuer.did, VC_STATUS.ACTIVE,
        state.issuerKey1, state.data.nonce, jwtVc)
      console.log('Add VC Status: ', JSON.stringify(vcAddStatus.data, null, 2))

      const nonce3 = await DIDNonce.getDIDNonce(state.issuer.did)
      console.log('Nonce 3: ', JSON.stringify(nonce3.data, null, 2))
      expect(nonce3.status).toEqual(200)
      state.data.nonce = nonce3.data.nonce

      const tag1 = faker.name.firstName() + `'sPersonalVC`
      const tags = [tag1]
      const cids = [state.cid]
      const addTags = await VCStatus.Tags(cids, tags, state.issuer.did,
        state.issuerKey2, state.data.nonce)
      console.log('Add tags: ', JSON.stringify(addTags.data, null, 2))
      expect(addTags.status).toEqual(400)
    } catch (err) {
      console.log(err.response)
      console.log(err.response.data)
      expect(err).toBeTruthy()
      expect(err.response.status).toEqual(400)
      expect(err.response.data.code).toBe(INVALID.SIGNATURE.CODE)
      expect(err.response.data.message).toBe(INVALID.SIGNATURE.MESSAGE)
    }
  })
})
